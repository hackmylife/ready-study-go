...
func TestTransfer(t *testing.T) {
	...
	dbtest.Exec(t, db, `CREATE TABLE transfer_events(from_id text,to_id text,amount bigint CHECK(amount>0))`)
...
func TestAuditAtomicity(t *testing.T) {
	db := dbtest.Open(t)
	dbtest.Accounts(t, db)
	dbtest.Exec(t, db, `CREATE TABLE transfer_events(from_id text,to_id text,amount bigint CHECK(amount<=10))`)
	wantError(t, Transfer(context.Background(), db, "a", "b", 20))
	equal(t, dbtest.Balance(t, db, "a"), int64(100))
	noError(t, Transfer(context.Background(), db, "a", "b", 10))
	var from, to string
	var amount int64
	noError(t, db.QueryRow(`SELECT from_id,to_id,amount FROM transfer_events`).Scan(&from, &to, &amount))
	equal(t, from, "a")
	equal(t, to, "b")
	equal(t, amount, int64(10))
}
